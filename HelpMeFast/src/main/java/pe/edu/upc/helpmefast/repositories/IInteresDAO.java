...
public interface IInteresDAO extends JpaRepository<Interes,Integer> {
    ...
    @Query(value ="Select aj.nombre as \"Nombre del paciente\", i.level_interes as \"Nivel de Interes\", s.name_servicios as \"Nombre del Servicio\", a.fecha_publicacion as \"Fecha de publicacion del servicio\", e.name_enfermero as \"Nombre del Enfermero\", count(c.id_comentarios) as \"Numero de comentarios\" from interes i, adulto_joven aj, anuncio a, servicios s, enfermero e, comentarios c where i.id_adulto_joven=aj.id_adulto_joven AND i.id_anuncio=a.id_anuncio AND a.id_servicios=s.id_servicios AND s.id_enfermero=e.id_enfermero AND s.id_servicios=c.id_servicios AND i.level_interes like '%alto' group by aj.nombre, i.level_interes, s.name_servicios, a.fecha_publicacion,e.name_enfermero order by a.fecha_publicacion desc" ,nativeQuery = true)
